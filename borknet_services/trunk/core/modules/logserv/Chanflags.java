	...
	public void parse_command(Core C, Logserv Bot, String numeric, String botnum, String username, String params)
	{
		...
		if(dbc.getAuthLev(username) >99)
		{
			...
			try
			{
				...
				Pattern pat = Pattern.compile("[^dn]");
				...
				if(dbc.setChanFlags(channel,flags))
				{
					if(flags.equals("n"))
					{
						C.cmd_privmsg(numeric, botnum, channel, "Logging normally.");
					}
					else if(flags.equals("d"))
					{
						C.cmd_privmsg(numeric, botnum, channel, "Deaf mode is now enabled, no logging!");
					}
