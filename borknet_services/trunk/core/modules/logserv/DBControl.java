...
public class DBControl
{
	 ...
	 * @param B				Main bot
	 */
	public DBControl(Core C, Logserv Bot, Connection con)
	{
		try
		{
			this.C = C;
	 ...
	 * @return			true or false
	 */
	public boolean LchanExists(String chan)
	{
		try
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("SELECT name FROM l_channels WHERE name = ?");
			pstmt.setString(1,chan);
			ResultSet rs = pstmt.executeQuery();
			rs.first();
			String channel = rs.getString(1);
	...
	public int getAuthLev(String numer)
	{
		...
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("SELECT authnick FROM users WHERE BINARY numer = ?");
	...
	public String[] getChanTable()
	{
		...
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("SELECT name FROM l_channels");
			ResultSet rs = pstmt.executeQuery();
	...
	public String getChanFlags(String channel)
	{
		...
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("SELECT flags FROM l_channels WHERE name = ?");
	...
	public void delChan(String chan)
	{
		...
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("DELETE FROM l_channels WHERE name = ? LIMIT 1");
			pstmt.setString(1,chan);
			pstmt.executeUpdate();
		}
	...
	public void addChan(String chan, String flags)
	{
		...
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("INSERT INTO l_channels VALUES ('',?,?)");
			pstmt.setString(1,chan);
	...
	public boolean setChanFlags(String chan, String flags)
	{
		...
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("UPDATE l_channels SET flags = ? WHERE name = ?");
	...
	public void clean()
	{
		try
		{
			...
			while(rs.next())
			{
				if(getChanUsers(rs.getString(1)) < 1)
				{
					delChan(rs.getString(1));
				}
			}
		}
		catch(Exception e)
	 ...
	 * @return			an array of all fields
	 */
	public String getUserNick(String numer)
	{
		try
		{
			PreparedStatement pstmt;
			pstmt = con.prepareStatement("SELECT nick FROM users WHERE BINARY numer = ?");
			pstmt.setString(1,numer);
			ResultSet rs = pstmt.executeQuery();
			rs.first();
