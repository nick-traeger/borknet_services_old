 ...
 */
public class Linkhost implements Command
{
    /**
     * Constructs a Loader
	...
	public void parse_command(Core C, V Bot, String numeric, String botnum, String username, String params)
	{
		...
		if(Integer.parseInt(auth[3]) >1)
		{
			...
			try
			{
				...
				String host[] = result[2].split("@");
				if(host[0].matches("[\\w]*") && host[1].matches("[\\w.]*"))
				{
					C.get_dbc().setAuthField(forceuser,8,result[2]);
			...
			{
				C.cmd_notice(numeric, botnum, username, "/msg "+Bot.get_nick()+" linkhost <#auth> <host>");
				return;
			}
		}
	...
	public void showcommand(Core C, V Bot, String numeric, String botnum, String username, int lev)
	{
		if(lev>1)
		{
			C.cmd_notice(numeric, botnum, username, "LINKHOST            Links a custom host to an account - level 2.");
		}
	}
}