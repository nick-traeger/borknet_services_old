 ...
 */
public class Stopqueue implements Command
{
    /**
     * Constructs a Loader
		...
		if(dbc.getAuthLev(username) >1)
		{
			...
			try
			{
				...
				if(Bot.onChan(chan))
				{
					...
					C.cmd_privmsg(numeric, botnum,chan, "The queue has now been deactivated.");
					Bot.addQueue(chan);
			...
			{
				C.cmd_notice(numeric, botnum,username, "/msg " + Bot.get_nick() + " stopqueue <#channel>");
				return;
			}
		}
	...
	public void showcommand(Core C, H Bot, String numeric, String botnum, String username, int lev)
	{
		if(lev > 1)
		{
			C.cmd_notice(numeric, botnum, username, "STOPQUEUE           Stops the queue. - level 2");
		}
	}
}